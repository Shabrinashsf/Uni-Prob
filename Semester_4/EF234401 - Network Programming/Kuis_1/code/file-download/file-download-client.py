...
class Client:
    ...
    def parse_header(self, header_content):
        ...
        filename = header.split(",")[0].split(":", 1)[1].strip()

        # get filesize from the header (use split string)
        filesize = int(header.split(",")[1].split(":", 1)[1].strip())
...
def start_client():
    ...
    status = client.send_message(message).strip()
    print(status)
    ...
    with open(file_path, 'wb') as f:
        ...
        if content_length > 0:
            ...
            # write to file
            # content is a string, write its bytes
