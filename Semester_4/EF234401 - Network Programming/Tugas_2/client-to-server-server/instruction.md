...
- Server Program:
  The server listens on a specified localhost and port, accepts a client connection, receives a message, prints the message to the console in the specified format, and then closes the connection. The server must handle at least one client connection and print the received message before shutting down for this problem.
